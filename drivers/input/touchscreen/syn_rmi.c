...
static void ts_work_func(struct work_struct *work)
{
	...
	if (ret < 0) {
		printk("%s i2c_smbus_read_i2c_block_data fail\n", __func__);
		goto touch_recovery;
	}
...
touch_recovery:
	printk("%s touch ic reset for recovery\n", __func__);
	esd_recovery(ts);
	if (ts->use_irq && irq_flag == 0) {
		irq_flag++;
		enable_irq(ts->client->irq);
	}
	ret = i2c_smbus_read_byte_data(ts->client, INT_STATUS);
	if (ret < 0)
		printk("%s touch ic recovery was failed\n", __func__);
	melt_mode = 1;
